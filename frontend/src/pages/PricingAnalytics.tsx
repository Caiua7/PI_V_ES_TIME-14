...
import { analyticsService, pricingService } from '../services/pricingService'
...
type PointLabelProps = { x?: number | string; y?: number | string; value?: number | string | boolean | null; index?: number }

function renderPricePointLabel({ x, y, value, index }: PointLabelProps) {
  const nx = Number(x), ny = Number(y)
  if (!Number.isFinite(nx) || !Number.isFinite(ny) || typeof value !== 'number') return null
  return <text x={index === 0 ? nx + 10 : nx} y={ny - 10} fill="#8b8b8b" fontSize={14} textAnchor={index === 0 ? 'start' : 'middle'}>{formatCurrency(value)}</text>
}

function renderMarginPointLabel({ x, y, value, index }: PointLabelProps) {
  const nx = Number(x), ny = Number(y)
  if (!Number.isFinite(nx) || !Number.isFinite(ny) || typeof value !== 'number') return null
  return <text x={index === 0 ? nx + 10 : nx} y={ny - 10} fill="#8b8b8b" fontSize={14} textAnchor={index === 0 ? 'start' : 'middle'}>{formatPercent(value)}</text>
}
...
export default function PricingAnalyticsPage() {
  ...
  useEffect(() => {
    void pricingService.getAll().then(setRows)
  }, [])

  // Carrega analytics sempre que filtros mudam
  useEffect(() => {
    void (async () => {
      setLoading(true)
      try {
        const filters = {
          ...
          date_from: dateStart ? dateStart.slice(0, 7) : undefined,
          date_to: dateEnd ? dateEnd.slice(0, 7) : undefined,
        }

        const [evolution, cardsData] = await Promise.all([
          analyticsService.getEvolution(filters),
          analyticsService.getCards(filters),
        ])

        setEvolutionData(
          evolution.series.map((point) => ({
            month: formatShortMonth(point.mes),
            avgPrice: point.preco,
            avgMargin: point.margem,
          }))
        )
        setCards(cardsData)
      } finally {
  ...
  }, [selectedSKU, selectedCategory, dateStart, dateEnd])

  const clientOptions = [...new Set(rows.map((r) => r.cliente))].map((v) => ({ value: v, label: v }))
  const skuOptions = [...new Set(rows.map((r) => r.sku))].map((v) => ({ value: v, label: v }))
  const categoryOptions = [...new Set(rows.map((r) => r.categoria))].map((v) => ({ value: v, label: v }))
  const subcategoryOptions = [...new Set(rows.map((r) => r.subcategoria))].map((v) => ({ value: v, label: v }))
  const sizeOptions = [...new Set(rows.map((r) => r.tamanho))].map((v) => ({ value: v, label: v }))
      ...
      <div className="max-w-[110rem] mx-auto px-6 py-4">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm mb-6 card-pronutrition hover-lift">
          <div className="flex items-center gap-4 mb-4">
            <h3 className="text-lg font-semibold flex items-center gap-2 text-gray-900"><Filter size={20} />Filtros de Análise</h3>
            {hasFilters && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className="block text-sm font-medium mb-2 text-gray-700">SKU</label>
